...
export default class SignUpSecurity extends Component {
  constructor(){
    super();
    this.state = {
      first_password: '',
  ...
  checkState = () => {
    if (this.state.first_password != '' && this.state.password != '' && this.state.password == this.state.first_password) {
      this.addToState()
      this.props.navigation.navigate("SignUpAbout", this.state)
    } else {
      if (this.state.first_password == '') {
        this.first_password.shake()
      } 
      if (this.state.password == '') {
        this.password.shake()
      }
      if (this.state.password != this.state.first_password) {
        this.first_password.shake()
        this.password.shake()
      }
    }
         ...
         <View style={styles.loginContainer} >
          ...
          <View style={styles.formContainer}>
            ...
            <FormInput 
              ...
              ref={input => this.first_password = input}
              secureTextEntry={true}
              onChangeText={(text) => this.setState({first_password: text, password: this.state.password})}
            />

           <Text style={styles.loginText}> password (again) </Text>
            <FormInput 
              containerStyle={styles.forminput}
              inputStyle={{color:Colors.accent}}
              ref={input => this.password = input}
              secureTextEntry={true}
              onChangeText={(text) => this.setState({first_password: this.state.first_password, password: text})}
            />
          </View>
        </View>
